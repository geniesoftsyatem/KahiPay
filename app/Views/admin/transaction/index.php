            ...
            <div class="card customer-card">
                <div class="card-body">
                    <form action="">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label for="txtsearch" class="form-label">Search Transactions</label>
                                <input class="form-control" name="txtsearch" type="text" value="<?= isset($txtsearch) ? esc($txtsearch) : ''; ?>" placeholder="Search by name, email, phone, or description...">
                            </div>
                            <div class="col-md-4 d-flex align-items-end">
                                ...
                                <a href="<?= site_url('wallet-transactions'); ?>" class="btn btn-light waves-effect">
                                    ...
                                    <tbody>
                                        <?php foreach ($results as $item) { ?>
                                            <tr>
                                                ...
                                                <td><?= esc($item->user_name); ?> <br> <small><?= esc($item->email); ?></small></td>
                                                <td class="text-center"><span class="badge bg-info"><?= ucfirst($item->type); ?></span></td>
                                                <td class="text-center"><?= number_format($item->amount, 2); ?></td>
                                                <td><?= esc($item->description); ?></td>
                                                <td class="text-center">
                                                    <span class="badge bg-<?= strtolower($item->status) === 'success' ? 'success' : 'warning' ?>">
                                                        <?= ucfirst($item->status); ?>
                                                    </span>
                                                </td>
                                                <td class="text-center"><?= date('d M Y, h:i A', strtotime($item->created_at)); ?></td>
                                                <td class="text-center">
                                                    <a href="<?= site_url("wallet-transactions/view/" . $item->transaction_id) ?>" class="btn btn-warning btn-sm" data-bs-toggle="tooltip" title="View Transaction">
